...
class CVTest(unittest.TestCase):
    ...
    def test_reproducibility(self):
        run1 = cv.cv_independent_associations(self.testcase_df, random_state=np.random.RandomState(0))
        run2 = cv.cv_independent_associations(self.testcase_df, random_state=np.random.RandomState(0))
    ...
    def test_randomness(self):
        ...
        while True:
            if attempt == max_attempts:
                ...
                try:
                    run1 = cv.cv_independent_associations(self.testcase_df)
                    run2 = cv.cv_independent_associations(self.testcase_df)
                    for first, second in zip(run1, run2):
                        train_first, test_first = first
                        train_second, test_second = second
    ...
    def test_cv_fold_count(self):
        run = cv.cv_independent_associations(self.testcase_df, cv_folds=3)
        self.assertEqual(3, sum([1 for _ in run]))

    def test_train_test_independence(self):
        _ = self.testcase_df['gene'].unique()
        _ = self.testcase_df['disease'].unique()
        run = cv.cv_independent_associations(self.testcase_df)
        for train, test in run:
            train_df = self.testcase_df.iloc[train, :]
            train_genes = train_df['gene'].unique()
            train_diseases = train_df['disease'].unique()
            test_df = self.testcase_df.iloc[test, :]
            test_genes = test_df['gene'].unique()
            test_diseases = test_df['disease'].unique()
    ...
    def test_train_test_independence_completeness(self):
        all_genes = self.testcase_cross_df['gene'].unique()
        all_diseases = self.testcase_cross_df['disease'].unique()
        run = cv.cv_independent_associations(self.testcase_cross_df)
        for train, test in run:
